...
*/
struct BankInfo {
  BankInfo(size_t size, unsigned int bank_id,
           std::vector<unsigned int> mirror_ids)
      : size_(size), bank_id_(bank_id), mirror_ids_(mirror_ids){};
  size_t size_;
...
struct Bank {
  Bank(size_t numLines, unsigned int bankNum) : size_(numLines), bankNum_(bankNum) {
    lines_.reserve(numLines);
  };
  Bank(size_t numLines, unsigned int bankNum,
       std::vector<unsigned int> bankMirrors)
      : size_(numLines), bankMirrors_(bankMirrors), bankNum_(bankNum) {
    lines_.reserve(numLines);
  }; // std::move ?
...
public:
  ...
  RAM(unsigned int banks, size_t lines)
  noexcept : banks_(banks), lines_(lines){};

  RAM(BanksAndSize &info, std::vector<int>* tracedLines = nullptr)
  noexcept : banks_(info.first.size()), lines_(info.second) {
