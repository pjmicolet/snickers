...
RAM::RAM( size_t ramSize, int banks, Mirrors mirrors ) noexcept : banks_(banks), lines_( ramSize/banks) {
    ram_.reserve( banks );
...
RAM::RAM( size_t ramSize, std::vector<int>& tracedLines, int banks ) noexcept : banks_(banks), lines_( ramSize/banks ) {
    ram_.reserve( banks );
    int ramLines = ramSize/banks;
    int tracing = 0;
    int lines = 0;
...
auto RAM::store( const int index, const std::byte data ) noexcept(DONT_THROW) -> void {
    auto bankIndex = calculateBank( index );
