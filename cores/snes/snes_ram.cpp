...
SNES_RAM::SNES_RAM(BanksAndSize &info, std::vector<int> *tracedLines /*= nullptr*/) noexcept : RAM(info, tracedLines) {}
...
auto SNES_RAM::calculateBank(unsigned int index) const noexcept(DONT_THROW) -> bankIndexPair {
...
auto SNES_RAM::addressToBank(const unsigned int address) const noexcept(DONT_THROW) -> unsigned int {
  auto bankIndex = calculateBank(address);
  return bankIndex.first;
}

auto SNES_RAM::store(const unsigned int index, const std::byte data) noexcept(DONT_THROW) -> void {
